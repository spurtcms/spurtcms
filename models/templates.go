...
func GetTemplateModuleList(keyword string, tenantid string, channelSlug string) (tempModuleList []TblTemplateModules, count int64, err error) {
	...
	if channelSlug != "" {
		query = query.Where("is_active = 1 and tenant_id is Null and slug_name=?", channelSlug)
		fmt.Println("channelSlug:", query)

	} else {
		query = query.Where("is_active = 1 and tenant_id is Null")
	}
	...
	fmt.Println("tempModuleList:", tempModuleList)
...
func GetChannelBasedTemplateCount(slugName string, tenantid string) (templatesCount TemplatesCount, err error) {
	...
	query := DB.Debug().Table("tbl_templates").Select("count(id) as template_count, slug_name").Where("is_deleted = 0 AND is_active = 1 AND tenant_id is Null and slug_name = ?", slugName)

	err = query.Group("slug_name").Find(&count).Error
	if err != nil {
...
func GetChannelDetailsWithTemplateCount(channelSlug string, tenantId string) (channelDetails TblChannel, err error) {

	query := DB.Debug().Where("is_deleted = 0 and is_active = 1 and tenant_id = ? and slug_name = ? ", tenantId, channelSlug).First(&channelDetails)
	if query.Error != nil {
		return TblChannel{}, query.Error
	}

	templateCount, err := GetChannelBasedTemplateCount(channelSlug, tenantId)
	if query.Error != nil {
		return TblChannel{}, query.Error
	}
...
func BlockLists(limit, offset int, filter Filter, tenantid string) (block []TblBlock, Totalblock int64, err error) {
