...
func Entries(c *gin.Context) {
	...
	channeldataFroFilter, entrcount, _, _ := ChannelConfig.ChannelEntriesList(chn.Entries{ChannelId: id, Limit: 0, Offset: 0}, TenantId)

	var allchnentry []chn.Tblchannelentries
	var responseCount []forms.FormResponseCount

	seenCats := make(map[int]bool)
	var uniqueCats []categories.TblCategories
	for _, entry := range channeldataFroFilter {

		for _, catGroup := range entry.Categories {
			if len(catGroup) == 0 {
				continue
			}

			lastCat := catGroup[len(catGroup)-1]

			// add globally (only once)
			if !seenCats[lastCat.Id] {
				seenCats[lastCat.Id] = true
				uniqueCats = append(uniqueCats, lastCat)
			}
		}
	}
	...
	for index, entry := range chnentry {
		...
		for _, catGroup := range entry.Categories {
			...
			lastCat := catGroup[len(catGroup)-1]
			entry.Categoriesnew = append(entry.Categoriesnew, lastCat)
...
func CreateEntry(c *gin.Context) {
	...
	roleslist, _, _ := MemberConfig.ListMemberGroup(mem.MemberGroupListReq{}, TenantId)
	var (
	...
	logoArray := []string{"Go", "Express", "Hugging-Face", "Next", "Node", "Open-AI", "React"}

	c.HTML(200, "addentry.html", gin.H{"Menu": menu, "roleslist": roleslist, "Ctalist": FormsBuidlersList, "articlecount": UserDetails.ArticleCount, "EntryId": id, "linktitle": "Create Entry", "title": ModuleName, "Fields": field, "translate": translate, "csrf": csrf.GetToken(c), "channellist": channelist, "CategoryName": FinalSelectedCategories, "entrycount": entrcount, "HeadTitle": translate.Channell.Channels, "Cmsmenu": true, "Entriestab": true, "currentdate": cdate, "StorageType": selectedtype.SelectedType, "blocks": string(bytedata1), "defaultblocks": string(bytedata2), "Mode": "create", "Slchannelid": slchannelid, "Storagepath": string(ubyte), "uploadurl": newpath, "Membershiplevellist": MembershiplevelList, "OwnApiKey": OwnApiKey, "logoArray": logoArray})
...
func AllEntries(c *gin.Context) {
	...
	channeldataFroFilter, Totalentris, _, _ := ChannelConfig.ChannelEntriesList(chn.Entries{ChannelId: 0, Limit: 0, Offset: 0, Keyword: filters.Keyword, ChannelName: filters.ChannelName, Title: filters.Title, Status: entrystatus, Sorting: sorting, EntriesTitle: entriestitle}, TenantId)
...
func DeleteEntries(c *gin.Context) {
	...
	entryDetails, _, err := ChannelConfig.FetchChannelEntryDetail(chn.EntriesInputs{TenantId: TenantId}, []int{})
	if err != nil {
	...
	err = ChannelConfig.DeleteGenericRouteslug("Entries", entryId, TenantId, userid)
	if err != nil {
	...
	go func() {
		...
		role, _ := NewRole.GetRoleById(c.GetInt("role"), TenantId)

		// channelDetails, _ := ChannelConfig.ChannelDetail(chn.Channels{Id: entries.ChannelId, TenantId: TenantId})

		data := map[string]interface{}{"entry title": entryDetails.Title, "entry's html content": entryDetails.Description, "channel name": channame, "event initiator name": userDetails.Username, "event initiator role": role.Name}

		wc := wh.WebhookClient{TenantId: c.GetString("tenant_id"), EventType: wh.EntryDelete, Data: data}

		_, ok := wc.HandleWebhooks()
...
func EntryStatus(c *gin.Context) {
	...
	go func() {
		...
		entryDetails, _, _ := ChannelConfig.FetchChannelEntryDetail(chn.EntriesInputs{TenantId: TenantId}, []int{})

		data := map[string]interface{}{"entry title": entryDetails.Title, "entry's html content": entryDetails.Description, "channel name": channelname, "event initiator name": userDetails.Username, "event initiator role": role.Name}

		var event wh.EventType

		switch status {

		case 1:

			event = wh.EntryPublish

		case 2:

			event = wh.EntryUnpubish

		default:

			event = wh.EntryCreate

		}

		wc := wh.WebhookClient{TenantId: c.GetString("tenant_id"), EventType: event, Data: data}
...
func PublishEntry(c *gin.Context) {
	...
	tech_stack := c.Request.PostFormValue("tech_stack")
	userid := c.GetInt("userid")
	ctaid, _ := strconv.Atoi(c.PostForm("ctaid"))
	access_type := c.PostForm("access_type")
	user_roleid, _ := strconv.Atoi(c.PostForm("user_roleid"))
	membershiplevel_id, _ := strconv.Atoi(c.PostForm("membershiplevel_id"))
	if access_type == "" {

		access_type = "every_one"
	}
	...
	entries := chn.EntriesRequired{
		...
		MembershipLevelId:  membershiplevel_id,
		TechStackLogos:     tech_stack,
	...
	if eid != 0 && Lang {
		...
		result, _ := ChannelConfig.CheckSlugInRoutes(eid, entries.SEODetails.MetaSlug, "Entries", TenantId)

		// result, err := ChannelConfig.GetSlugTypeFromDB(entries.SEODetails.MetaSlug)
		...
		if result.Id == 0 {
			...
			routeinput.ProductId = eid
			routeinput.Slug = strings.ReplaceAll(strings.ToLower(entries.SEODetails.MetaSlug), " ", "-")
			routeinput.TenantId = TenantId
			routeinput.ModuleName = "Entries"
			...
			rerr := ChannelConfig.UpdateGenericRouteslug(routeinput)
		...
		Totalentries, _, _, _ := ChannelConfig.ChannelEntriesList(chn.Entries{ChannelId: 0, Limit: 0, Offset: 0}, TenantId)
		...
		var routeinput chn.TblRouteSlugs
		routeinput.ProductId = chenid.Id
		routeinput.Slug = chenid.Slug
		routeinput.TenantId = TenantId
		routeinput.ModuleName = "Entries"
		routeinput.CreatedBy = userid
		_, err := ChannelConfig.CreateGenetricRouteslug(routeinput)
	...
	go func() {
		...
		data := map[string]interface{}{"entry title": title, "entry's html content": text, "channel name": cname, "event initiator name": userDetails.Username, "event initiator role": role.Name}
...
func EditEntry(c *gin.Context) {
	...
	var filters memship.Filter
	MembershiplevelList, _, _ := MembershipConfig.MembershipLevelsList(0, 0, filters, TenantId)
	...
	c.HTML(200, "addentry.html", gin.H{"Menu": menu, "roleslist": roleslist, "Ctalist": FormsBuildersList, "Viewbaseurl": viewurl, "linktitle": "Edit Entry", "title": ModuleName, "Entries": entries, "Fields": field, "translate": translate, "csrf": csrf.GetToken(c), "channellist": channelist, "AllCategories": AllCategorieswithSubCategories, "HeadTitle": translate.Channell.Channels, "Cmsmenu": true, "Entriestab": true, "StorageType": selectedtype.SelectedType, "editbread": "Edit Entry", "blocks": string(bytedata), "Mode": "edit", "Slchannelid": slchannelid, "EntryId": slchannelid, "Storagepath": string(ubyte), "uploadurl": baseurl, "Membershiplevellist": MembershiplevelList, "defaultblocks": string(bytedata2), "Language": Language, "logoArray": logoArray})
...
func Updatechannelfields(c *gin.Context) {
	...
	metatitle := channeldetails.SeoTitle
	metadesc := channeldetails.SeoDescription
	metakey := channeldetails.SeoKeyword
	...
	if ederr := ChannelConfig.EditChannel(channelname, channeluniqueid, "", channeldesc, metatitle, metadesc, metakey, userid, channelid, categoryvalue, TenantId); ederr != nil {
...
func DeleteSelectedEntry(c *gin.Context) {
	...
	_, channelEntries, err := ChannelConfig.FetchChannelEntryDetail(chn.EntriesInputs{TenantId: TenantId}, entryids)
	...
	go func() {
		...
		for _, entry := range channelEntries {
			...
			channelDetails, _ := ChannelConfig.ChannelDetail(chn.Channels{Id: entry.ChannelId, TenantId: TenantId})

			data := map[string]interface{}{"entry title": entry.Title, "entry's html content": entry.Description, "channel name": channelDetails.ChannelName, "event initiator name": userDetails.Username, "event initiator role": role.Name}
