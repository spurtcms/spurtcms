...
func ChannelList(c *gin.Context) {
	...
	for _, val := range channelslist {
		...
		if len(strings.TrimSpace(first)) > 0 {
			firstn = strings.ToUpper(string([]rune(first)[0]))
		}

		var lastn string
		if len(strings.TrimSpace(last)) > 0 {
			lastn = strings.ToUpper(string([]rune(last)[0]))
		}
...
func CreateChannel(c *gin.Context) {
	...
	ChannelSlugName := c.Request.PostFormValue("ChannelSlugName")
	channeluniqueid := c.Request.PostFormValue("channeluniqueid")
	channeldesc := c.Request.PostFormValue("channeldesc")
	metatitle := c.Request.PostFormValue("metatitle")
	metadesc := c.Request.PostFormValue("metadesc")
	metakey := c.Request.PostFormValue("metakey")
	fmt.Println("metatitle::", metatitle, "metadesc::", metadesc, "metakey::", metakey)
	...
	if permisison {
		...
		newchannel, cerr := ChannelConfig.CreateChannel(chn.ChannelCreate{ChannelName: channelname, SlugName: ChannelSlugName, ChannelUniqueId: channeluniqueid, ChannelDescription: channeldesc, SeoTitle: metatitle, SeoDescription: metadesc, SeoKeyword: metakey, CategoryIds: categoryvalue, CreatedBy: userid}, moduleid, TenantId)
		...
		var ChannelRoute chn.TblRouteSlugs
		ChannelRoute.ProductId = newchannel.Id
		ChannelRoute.Slug = newchannel.SlugName
		ChannelRoute.TenantId = TenantId
		ChannelRoute.ModuleName = "Channel"

		_, errroute := ChannelConfig.CreateGenetricRouteslug(ChannelRoute)

		if errroute != nil {
			fmt.Println("Channel Route create Error : ", err)
		}
...
func UpdateChannel(c *gin.Context) {
	...
	if permisison {
		...
		if cerr := ChannelConfig.EditChannel(channelname, channeluniqueid, ChannelSlugName, channeldesc, metatitle, metadesc, metakey, userid, channelid, categoryvalue, TenantId); cerr != nil {
		...
		var ChannelRoute chn.TblRouteSlugs
		ChannelRoute.ProductId = channelid
		ChannelRoute.Slug = ChannelSlugName
		ChannelRoute.TenantId = TenantId
		ChannelRoute.ModuleName = "Channel"
		ChannelRoute.ModifiedBy = userid

		err := ChannelConfig.UpdateGenericRouteslug(ChannelRoute)
		if err != nil {
			fmt.Println("Channel slug Update Error : ", err)
		}
...
func DeleteChannel(c *gin.Context) {
	...
	if permisison {
		...
		err = ChannelConfig.DeleteGenericRouteslug("Channel", channelid, TenantId, userid)
		if err != nil {
...
func AddChannelTomyCollection(c *gin.Context) {
	...
	if permisison {
		...
		var ChannelRoute chn.TblRouteSlugs
		ChannelRoute.ProductId = newchannel.Id
		ChannelRoute.Slug = strings.ToLower(strings.ReplaceAll(newchannel.ChannelName, " ", "-"))
		ChannelRoute.TenantId = TenantId
		ChannelRoute.ModuleName = "Channel"
