...
func GenerateEmail(email, subject string, data map[string]interface{}, message string, wg *sync.WaitGroup) error {
	...
	value, ok := data["isactive"]

	if ok && value != nil {
		data1["IsActive"] = value.(string)
	}
	...
	senderDisplayName := os.Getenv("FROM_NAME")
...
func ConvertBase64(imageData string, storagepath string) (imgname string, path string, err error) {
	...
	// Extract MIME type: example → "image/png" or "image/svg+xml"
	mimeEnd := strings.Index(imageData, ";base64,")
	if mimeEnd == -1 {
		return "", "", fmt.Errorf("invalid base64 image data")
	}

	mimeType := imageData[5:mimeEnd] // skip "data:"
	// mimeType becomes: "image/png", "image/jpeg", "image/svg+xml"

	// Get the extension from MIME type
	ext := mimeToExt(mimeType)

	// Extract only the Base64 string (after comma)
	base64data := imageData[strings.IndexByte(imageData, ',')+1:]
...
func mimeToExt(mimeType string) string {
	...
	if strings.HasPrefix(mimeType, "image/") {
		mimeType = mimeType[6:] // remove "image/"
	}
...
func ConvertBase64toByte(imageData string, storagepath string) (imgname string, path string, imagebyte []byte, err error) {
	...
	ext := mimeToExt("image/" + imageData[metaStart+11:metaEnd])
	if ext == "" {
...
func ConvertBase64toXML(xmlData string, storagepath string) (xmlname string, xmlpath string, xmlbyte []byte, err error) {
	metaEnd := strings.Index(xmlData, ";base64,")
	if metaEnd == -1 {
		err = errors.New("invalid base64 XML data: missing ';base64,'")
		return "", "", nil, err
	}

	// Check for XML mime type
	metaStartXML := strings.Index(xmlData, "data:application/xml")
	if metaStartXML == -1 {
		metaStartXML = strings.Index(xmlData, "data:text/xml")
	}
	if metaStartXML == -1 {
		err = errors.New("invalid XML data: must be 'data:application/xml' or 'data:text/xml'")
		return "", "", nil, err
	}

	base64data := xmlData[metaEnd+8:]
	if base64data == "" {
		err = errors.New("no base64 XML data found")
		return "", "", nil, err
	}

	randNum := strconv.FormatInt(time.Now().UnixNano(), 10)
	xmlName := "sitemap-" + randNum + ".xml"
	xmlPath := strings.TrimRight(storagepath, "/") + "/" + xmlName

	decode, err := base64.StdEncoding.DecodeString(base64data)
	if err != nil {
		return "", "", nil, fmt.Errorf("XML base64 decode error: %w", err)
	}

	// fmt.Println("XML sitemap processed:", xmlName)
	return xmlName, xmlPath, decode, nil
}
...
func SendUserOtp(wg *sync.WaitGroup, data map[string]interface{}, email string, tenantid string, action string) {
	...
	var templates models.TblEmailTemplate
	err := models.GetTemplates(&templates, action, tenantid)
	if err != nil {
		fmt.Println("maiol sending error ")
	}
	if templates.IsActive == 1 {
		sub := templates.TemplateSubject
		msg := templates.TemplateMessage
...
func RepliedSuccessfully(wg *sync.WaitGroup, data map[string]interface{}, email string, tenant_id string, action string) {
	var templates models.TblEmailTemplate
	models.GetTemplates(&templates, "Replied Successfully", "")
	if templates.IsActive == 1 {
		sub := templates.TemplateSubject
		msg := templates.TemplateMessage
...
func ModuleRouteName(c *gin.Context) (modulename string, tabname string, moduleid int) {
	...
	if strings.Contains(routeName, "/website/pages") {

		routeName = "/pages"
	}

	if strings.Contains(routeName, "/website/menu") {

		routeName = "/menu"
	}

	if strings.Contains(routeName, "/website/seo") {
		routeName = "/seo"
	}
	...
	if strings.Contains(routeName, "/userrole") {

		routeName = "/userrole"

	}
	if strings.Contains(routeName, "/orders") {

		routeName = "/listing/"
	}
	if strings.Contains(routeName, "/downloads") {

		routeName = "/listing/"
	}
	if strings.Contains(routeName, "/payments") {

		routeName = "/listing/"
	}

	if strings.Contains(routeName, "/browsetheme") {

		routeName = "/browsetheme"
	}
...
func OrderPlacedNotification(wg *sync.WaitGroup, data map[string]interface{}, email string, template_name string) {
	var templates models.TblEmailTemplate
	models.GetTemplates(&templates, template_name, "")

	if templates.IsActive == 1 {
		sub := templates.TemplateSubject
		msg := templates.TemplateMessage
		var replacer *strings.Replacer
		if val, ok := data["isactive"].(string); ok {
			replacer = strings.NewReplacer(
				"{FirstName}", data["firstname"].(string),
				"{Email}", data["email"].(string),
				"{Title}", data["title"].(string),
				"{Loginurl}", data["login_url"].(string),
				"{AitoolsLogo}", data["aitools_logo"].(string),
				"{FbLogo}", data["fb_logo"].(string),
				"{LinkedinLogo}", data["linkedin_img"].(string),
				"{TwitterLogo}", data["twitter_img"].(string),
				"{YoutubeLogo}", data["youtube_logo"].(string),
				"{InstaLogo}", data["instagram_img"].(string),
				"{FacebookLogo}", data["facebook_img"].(string),
				"{InstagramLink}", data["instagram"].(string),
				"{YoutubeLink}", data["youtube"].(string),
				"{LinkedinLink}", data["linkedin"].(string),
				"{TwitterLink}", data["twitter"].(string),
				"{Isactive}", val,
				"{ListingsImg}", data["listings_img"].(string),
				"{Price}", data["price"].(string),
				"{AdminName}", data["admin_name"].(string),
				"{CheckIcon}", data["check_icon"].(string),
			)
		}
		...
		err := GenerateEmail(email, sub, data, msg, wg)
		if err != nil {
			ErrorLog.Printf("Cann't send your order related datas to your Email %s error:", err)

		}
	}
}

func PaymentSuccessNotification(wg *sync.WaitGroup, data map[string]interface{}, email string, template_name string) {
	var templates models.TblEmailTemplate
	models.GetTemplates(&templates, template_name, "")
	...
	if templates.IsActive == 1 {
		...
		if val, ok := data["isactive"].(string); ok {
			replacer = strings.NewReplacer(
				...
				"{Title}", data["title"].(string),
				"{AccessLink}", data["access_link"].(string),
				"{AdminDashboardLink}", data["admin_dashboard_link"].(string),
				"{Loginurl}", data["login_url"].(string),
				"{AitoolsLogo}", data["aitools_logo"].(string),
				"{FbLogo}", data["fb_logo"].(string),
				"{LinkedinLogo}", data["linkedin_img"].(string),
		...
		err := GenerateEmail(email, sub, data, msg, wg)
		if err != nil {
			ErrorLog.Printf("Cann't send your payment related details Email %s error:", err)

		}
	}
}

func FileNotFoundForTemplates(c *gin.Context) {

	c.HTML(200, "404-page-templates.html", nil)

}

func MemberWelcomeMail(wg *sync.WaitGroup, data map[string]interface{}, email, action string) {
...
func IntegrationandIdeaEmail(wg *sync.WaitGroup, data map[string]interface{}, email string, template_name string) {
	var templates models.TblEmailTemplate
	models.GetTemplates(&templates, template_name, "")
	...
	if templates.IsActive == 1 {
		...
		if val, ok := data["isactive"].(string); ok {
			replacer = strings.NewReplacer(
				...
				"{CustomerMessage}", data["customer_message"].(string),
				"{LinkedinLogo}", data["linkedin_img"].(string),
				"{TwitterLogo}", data["twitter_img"].(string),
				"{YoutubeLogo}", data["youtube_logo"].(string),
				"{InstaLogo}", data["instagram_img"].(string),
		...
		err := GenerateEmail(email, sub, data, msg, wg)
		if err != nil {
			ErrorLog.Printf("Cann't send your integration request Email %s error:", err)
		}
	}
}
...
func GetRouteSlugFormDB(RouteSlug string) (*chn.TblRouteSlugs, error) {

	result, err := ChannelConfig.GetSlugTypeFromDB(RouteSlug)

	if err != nil {

		return &chn.TblRouteSlugs{}, err
	}
	return result, nil

}

func CheckDupliateRoute(c *gin.Context) {
	...
	flg, err := ChannelConfig.CheckDupliateRoute(productid, slugname, moduleName, TenantId)

	if err != nil {
		ErrorLog.Printf("Genertic Route check error: %s", err)
...
func GetBuildVersion(filePath string) (string, error) {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "BUILD_VERSION=") {
			return strings.TrimSpace(strings.TrimPrefix(line, "BUILD_VERSION=")), nil
		}
	}
