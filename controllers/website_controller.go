...
func NewWebsitePage(c *gin.Context) {
	...
	baseDomain := GetBaseDomainFromUrl(baseURL)
	...
	if host == baseDomain || host == "www."+baseDomain {
		...
		if token != "" && token != nil && TENANTID != "1" {

			AuthenticatedUser(c, session)
			return
		} else if TENANTID == "1" {
...
func GetBaseDomainFromUrl(baseURL string) string {
...
func AuthenticatedUser(c *gin.Context, session *sessions.Session) {
...
func WebsiteLoad(c *gin.Context, Tenantid, subdomain string) {
	...
	filters.Title = strings.TrimSpace(c.Query("location"))
	filters.Keyword = strings.TrimSpace(c.Query("Title"))
	...
	if template_name != "support_sphere" {
		...
		for _, val := range newmenulist {
			if val.TypeId != 0 {
				...
				Count = count
				for i := range entries {
					entries[i].CreatedDate = entries[i].CreatedOn.In(TZONE).Format(Datelayout)
					...
					channelinfo, _, _ := ChannelConfigWP.GetChannelsById(val.TypeId, User.TenantId)
				...
				if len(chnentry) >= Limit {
					chnentry = chnentry[:Limit]
					break
				}
			}
		}
		...
		for i := range newmenulist {
			...
			if val.TypeId != 0 {
				...
				for i := range entries {
					channelinfo, _, _ := ChannelConfigWP.GetChannelsById(entries[i].ChannelId, User.TenantId)
	...
	Previous, Next, PageCount, Page := Pagination(pageno, int(Count), Limit)
...
func ContactusPage(c *gin.Context) {
	...
	captchaKey := os.Getenv("GOOGLE_RECAPTCHA_KEY")

	fmt.Println("captchaKey", captchaKey)

	c.HTML(200, "contactus.html", gin.H{"linktitle": "SpurtCMS Contact Us | Golang Open Source CMS Solutions", "description": "Connect with SpurtCMS – Your source for Golang Open Source CMS Solutions. Reach out through our Contact Us page for tailored content management system expertise.", "keywords": "Golang open source solution, Golang cms solutions, built cms website with Golang, CMS Golang Tech Stack, Golang headless cms, golang cms open source, Golang cms framework, Golang Customize CMS Solution.", "CountryList": countryList, "csrf": csrf.GetToken(c), "CaptchaKey": captchaKey})
}

func ContactusSuccess(c *gin.Context) {

	c.HTML(200, "contactus-submitted.html", gin.H{"linktitle": "SpurtCMS Contact Us | Golang Open Source CMS Solutions", "description": "Connect with SpurtCMS – Your source for Golang Open Source CMS Solutions. Reach out through our Contact Us page for tailored content management system expertise.", "keywords": "Golang open source solution, Golang cms solutions, built cms website with Golang, CMS Golang Tech Stack, Golang headless cms, golang cms open source, Golang cms framework, Golang Customize CMS Solution.", "csrf": csrf.GetToken(c)})
}
...
func GetProfileName(c *gin.Context, Tenantid string) (bool, member.Tblmember, string) {

	session, _ := Store.Get(c.Request, os.Getenv("TEMPLATE_SESSION_KEY"))

	var member_details member.Tblmember
	...
	if tkn != "" {
		...
		member_id, err := VerifyToken(tkn, os.Getenv("JWT_SECRET"))

		if err != nil {
			ErrorLog.Println("err", err)
		}
		member_details, err = MemberConfig.GetMemberDetails(member_id, Tenantid)
		if err != nil {
			fmt.Println(err.Error())
		}
		if member_details.ProfileImagePath == "" {

			if member_details.FirstName != "" {

				FirstName = string(member_details.FirstName[0])
			}
			if member_details.LastName != "" {
				LastName = string(member_details.LastName[0])

			}
			ProfileName = FirstName + LastName
			ProfileName = strings.ToUpper(ProfileName)
		}
		if member_id != 0 {
	...
	return profile, member_details, ProfileName
}
