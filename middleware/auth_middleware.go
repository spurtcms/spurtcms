...
func TemplateJWTAuth() gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		session, err := controllers.Store.Get(c.Request, os.Getenv("TEMPLATE_SESSION_KEY"))
		if err != nil {
			fmt.Println(err.Error() + "TJWT001")
		}

		tkn, ok := session.Values["token"]
		if !ok {
			fmt.Println("token error occur")
		}
		if tkn == nil {
			c.Abort()
		...
		if tkn != nil {
			...
			if err != nil {
				controllers.ErrorLog.Println("err", err.Error()+"TJWT002")
			}
