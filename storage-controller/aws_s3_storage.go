...
func SetS3value() {
	...
	integration, _ := models.GetS3Credential(TenantId, "AWS")
	type CredentialsWrapper struct {
		Credentials models.Credentials `json:"credentials"`
	}

	if integration.IsActive == 1 {
		var wrapper CredentialsWrapper
		if err := json.Unmarshal(integration.Credentials, &wrapper); err != nil {
			...
			creds := wrapper.Credentials
			AWSID = creds.AccessKey
			AWSKEY = creds.SecretKey
			AWSREGION = creds.Region
			AWSBUCKET = creds.BucketName
		}
